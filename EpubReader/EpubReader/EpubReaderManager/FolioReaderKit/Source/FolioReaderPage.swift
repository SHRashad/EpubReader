...
extension FolioReaderPage: WKNavigationDelegate{
    ...
    public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        guard let url = navigationAction.request.url else { decisionHandler(.cancel);
            return }
        
            if scheme == "highlight" || scheme == "highlight-with-note" {
                shouldShowBar = false
        
                guard let decoded = url.absoluteString.removingPercentEncoding else { decisionHandler(.cancel);
                    return }
